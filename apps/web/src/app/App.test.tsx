...
describe("App", () => {
  ...
  it("loads every cursor page in the local library", async () => {
    const testBridge = bridge();
    vi.mocked(testBridge.queryLibrary)
      .mockResolvedValueOnce({
        items: [document({ id: "document-1", title: "First paper" })],
        nextCursor: "100",
      })
      .mockResolvedValueOnce({
        items: [document({ id: "document-2", title: "Later paper" })],
        nextCursor: null,
      });
    ...
    expect(testBridge.queryLibrary).toHaveBeenNthCalledWith(2, {
      sort: "recent",
      limit: 100,
      cursor: "100",
    });
  ...
  it("imports PDFs selected by the native picker and refreshes the list", async () => {
    ...
    const file = new File(["%PDF"], "atlas.pdf", { type: "application/pdf" });
    vi.mocked(testBridge.pickPdfFiles).mockResolvedValue([file]);
    ...
    expect(testBridge.pickPdfFiles).toHaveBeenCalledWith(true);
    expect(testBridge.importPdf).toHaveBeenCalledWith(file);
  ...
  it("keeps search disabled while an import owns the operation lock", async () => {
    ...
    vi.mocked(testBridge.pickPdfFiles).mockResolvedValue([
  ...
  it("imports dropped PDF files and shows drag feedback", async () => {
    const testBridge = bridge();
    const imported = document();
    ...
    act(() => dropListener?.({ type: "enter" }));
    expect(screen.getByText("Drop to import")).toBeInTheDocument();
    act(() => dropListener?.({ type: "drop", files: [file] }));
  ...
  it("removes the library record and its managed copy after confirmation", async () => {
    const testBridge = bridge();
    ...
    expect(screen.getByRole("status")).toHaveTextContent("managed PDF copy was removed");
  });
  ...
  it("relocates a missing source after verifying the selected PDF", async () => {
    ...
    const file = new File(["%PDF"], "restored.pdf", { type: "application/pdf" });
    vi.mocked(testBridge.pickPdfFiles).mockResolvedValue([file]);
    ...
    expect(testBridge.pickPdfFiles).toHaveBeenCalledWith(false);
    expect(testBridge.relocateDocument).toHaveBeenCalledWith("document-1", file);
  });
  ...
  it("reports an import failure without discarding the current library", async () => {
    const testBridge = bridge();
    vi.mocked(testBridge.pickPdfFiles).mockResolvedValue([
      new File(["not a PDF"], "not-a-pdf.txt", { type: "text/plain" }),
    ]);
