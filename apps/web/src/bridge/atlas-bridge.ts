...
export type PdfDropEvent = { type: "enter" } | { type: "drop"; files: File[] } | { type: "leave" };
...
export interface AtlasBridge {
  pickPdfFiles(multiple: boolean): Promise<File[]>;
  ...
  importPdf(file: File): Promise<ImportPdfResult>;
  queryLibrary(input: LibraryQuery): Promise<LibraryPage>;
  refreshLibrarySources(): Promise<RefreshSourcesResult>;
  relocateDocument(documentId: DocumentId, file: File): Promise<DocumentSummary>;
