...
export function useLibrary(bridge: AtlasBridge) {
  ...
  const queryDocuments = useCallback(
    async (text = searchText) => {
      setDocuments(await queryAllDocuments(bridge, text));
      setError(undefined);
    },
    [bridge, searchText],
  );
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const documents = await queryAllDocuments(bridge);
        if (!active) {
          return;
        }
  ...
  const importFiles = useCallback(
    async (files: File[]) => {
      ...
      if (uniqueFiles.length === 0 || operationLock.current) {
        return;
      }
      ...
      try {
        for (const file of uniqueFiles) {
          try {
            const result = await bridge.importPdf(file);
            if (result.duplicate) {
              duplicates += 1;
            } else {
              imported += 1;
  ...
  const importFromPicker = useCallback(async () => {
    try {
      await importFiles(await bridge.pickPdfFiles(true));
    } catch (reason) {
  ...
  const relocate = useCallback(
    async (document: DocumentSummary) => {
      ...
      try {
        const [file] = await bridge.pickPdfFiles(false);
        if (!file) {
          return;
        }
        operationLock.current = true;
        ...
        await bridge.relocateDocument(document.id, file);
        await queryDocuments();
        setNotice({
          kind: "success",
  ...
  const remove = useCallback(
    async (document: DocumentSummary) => {
      ...
      try {
        ...
        setNotice({
          kind: "success",
          message: `Removed “${document.title}” from Atlas Reader. Its managed PDF copy was removed.`,
        });
      } catch (reason) {
        setNotice({ kind: "error", message: errorMessage(reason) });
      } finally {
...
async function queryAllDocuments(bridge: AtlasBridge, text = ""): Promise<DocumentSummary[]> {
  ...
  do {
    ...
    if (cursor && !seenCursors.add(cursor)) {
      throw new Error("Atlas library pagination returned a repeated cursor");
    }
  } while (cursor);
  return items;
}
